...
% Weights parameters for the dpli-dri calculation
w1 = 1.0;
w2 = 1.0;
w3 = 1.0;
...
% subplots per figure we are also iterating over all the shift_weight to
% test the different shift that are possible
for w = 1:length(shift_weights)
    ...
    title(sprintf("WSAS dpli-dri for alpha (attempt #6) weight=%d",shift_weight));

    % Save it to disk
    filename = sprintf("%sdpli_dri_w_%d_6.png",OUT_DIR,shift_weight);
