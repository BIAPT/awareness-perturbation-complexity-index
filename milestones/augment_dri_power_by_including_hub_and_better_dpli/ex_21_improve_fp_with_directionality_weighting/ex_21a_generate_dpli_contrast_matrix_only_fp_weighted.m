...
OUT_DIR = "/media/yacine/My Book/result_dri/fp_w/";
...
% this is used in the definition of the contrast matrix to scale the tanh function
% We are using 1 2 10 100 weights to be able to see if there is a big
% difference in the matrices generated
shift_weights = [1 2 10 100];

%% Creating the figures
% Here we iterate over each participant and each epochs to create the 3
% subplots per figure
for w = 1:length(shift_weights)
    shift_weight = shift_weights(w);
    for p = 1:length(P_ID)
        participant = P_ID{p};
        disp(participant);
        ...
        baseline_vs_recovery = calculate_contrast_matrix(baseline_f_dpli, recovery_f_dpli, shift_weight);
        baseline_vs_anesthesia = calculate_contrast_matrix(baseline_f_dpli, anesthesia_f_dpli, shift_weight);
        recovery_vs_anesthesia = calculate_contrast_matrix(recovery_f_dpli, anesthesia_f_dpli, shift_weight);
        ...
        title(sprintf("%s alpha FP-W(%d) B vs R", participant, shift_weight))
        subplot(1,3,2)
        plot_pli_diff_axis(baseline_vs_anesthesia, x_region, y_region, sim_all, COLOR)
        title(sprintf("%s alpha FP-W(%d) B vs A", participant, shift_weight))
        subplot(1,3,3)
        plot_pli_diff_axis(recovery_vs_anesthesia, x_region, y_region, sim_all, COLOR)
        title(sprintf("%s alpha FP-W(%d) R vs A", participant, shift_weight))
        colorbar;
        set(handle, 'Position', [70,152,1527,589]);

        filename = sprintf("%s%s_alpha_weighted_%d_contrast_fp_dpli.png", OUT_DIR, participant, shift_weight);
