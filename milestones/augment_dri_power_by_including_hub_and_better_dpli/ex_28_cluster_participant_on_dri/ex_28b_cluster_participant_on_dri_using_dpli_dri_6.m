...
% https://github.com/BIAPT/awareness-perturbation-complexity-index/issues/28
...
for b = 1:length(binary_thresholds)
    ...
    ylabel("Hub-DRI");
    colormap(COLOR);
    title(sprintf("WSAS Both DRI for alpha at t=%.1f(attempt #2)",threshold));

    % Save it to disk
    filename = sprintf("%sboth_dri_%.1f_2.png",OUT_DIR,threshold);
