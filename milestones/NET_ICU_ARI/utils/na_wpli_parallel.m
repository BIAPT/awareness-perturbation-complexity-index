function [result] = na_wpli_parallel(recording, frequency_band, window_size, step_size, number_surrogate, p_value)
    ...
    parfor win_i = 1:number_window
        disp(strcat("wpli at window: ",string(win_i)," of ", string(number_window)));         % Calculate the wpli 
        segment_data = squeeze(windowed_data(win_i,:,:));
        wpli_tofill(win_i,:,:) = wpli(segment_data, number_surrogate, p_value);
    end
