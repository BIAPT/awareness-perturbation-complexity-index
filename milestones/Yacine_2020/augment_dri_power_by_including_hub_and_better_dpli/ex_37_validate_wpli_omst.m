...
% https://github.com/BIAPT/awareness-perturbation-complexity-index/issues/37
...
for p = 1:length(P_ID)
    ...
    % Threshold using omst
    [~, baseline_r_wpli, ~, ~, ~, ~]=threshold_omst_gce_wd(baseline_r_wpli,0);
    [~, anesthesia_r_wpli, ~, ~, ~, ~]=threshold_omst_gce_wd(anesthesia_r_wpli,0);
    if ~isempty(recovery_r_wpli)
        [~, recovery_r_wpli, ~, ~, ~, ~]=threshold_omst_gce_wd(recovery_r_wpli,0);
    end
