...
for p = 1:length(P_ID)
    ...
    handle = figure;
    subplot(1,3,1)
    plot_pli(baseline_f_dpli, common_location, dpli_all)
    title(strcat(participant, " alpha dpli at baseline"));
    subplot(1,3,2)
    plot_pli(anesthesia_f_dpli, common_location, dpli_all)
    title(strcat(participant, " alpha dpli at anesthesia"));
    subplot(1,3,3)
    plot_pli(recovery_f_dpli, common_location, dpli_all)
...
function [r_dpli, r_location, r_regions] = process_bp_dpli(filename, map_file)
   ...
   for i = 1:length(location)
      ...
      % Removing all the channels to the right of the brain
